...
const Dashboard = ( ) =>{
    ...
    return <main className="mx-auto max-w-7xl md:p-10">
        ...
        gap-4 border-b border-gray-200 pb-5 sm:flex-row sm:items-center
        sm:gap-0">
     ...
     {/* Display all user files */}
        {files && files?.length !== 0 ?(
            ...
            gap-6 divide-y divide-zinc-200 md:grid-cols-2 lg:grid-cols-3'>
                {files.sort((a,b) => new Date(b.createdAt).getTime() -
                    new Date(a.createdAt).getTime()).map((file) => (
                    <li key={file.id} className='col-span-1 divide-y-1
                    divide-gray-200 rounded-lg bg-white shadow transition
                    hover:shadow-lg'>
